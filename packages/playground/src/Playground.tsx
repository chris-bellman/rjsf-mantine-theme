...
import MantineForm from '@chris-bellman/rjsf-mantine-5-theme';
...
export function Playground() {
  ...
  const preset = (
    <Select
      ...
      data={sectionList.map((x) => x.items).flat()}
  ...
  const [theme, setTheme] = useState<string | null>(
    new URLSearchParams(window.location.search).get('theme') || 'Mantine'
  );
  useEffect(() => {
